...
#List of all the Restriction Enzymes currently in the program
...
def process_file():
    dst = "C:\\Users\\musta_000\\Desktop\\REID\\REID_v0.5.2\\"
...
def program():
    ...
    if CheckVar8.get() == 1:
        ...
        index1 += 1
    index3 = 0
    while index3 < len(REcurrentlist)-1:
        sheet = workbook.add_sheet(REcurrentlist[index3][0])
        ...
        if CheckVar9.get() == 1:
            ...
            column += 1
            for item in list:
                for usrseq in SeqIO.parse(item, "fasta"):
        ...
        if CheckVar10.get() == 1:
            ...
            for item in list:
                for usrseq in SeqIO.parse(item, "fasta"):
                    ...
                    sheet.write(row, column, item)
                    row += 1
                    writeList.append(gtc.find(REcurrentlist[index3][1], index2)+REcurrentlist[index3][2])
                    ...
                    while index2 < len(gtc):
                        ...
                        if index4 < 0:
                            break
                        writeList.append((index4 + REcurrentlist[index3][2])-(index2 + REcurrentlist[index3][2]))
                        index2 += 6
                    writeList.append(len(gtc)-(gtc.rfind(REcurrentlist[index3][1]) + REcurrentlist[index3][2]))
                    sheet.write(row, column, len(writeList))
                    row += 1
                    writeList.sort()
