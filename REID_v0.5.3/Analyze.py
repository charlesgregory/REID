...
for sheet in sheets:
    ...
    while index2 < repLen+2:
        ...
        if index4 == sheet.ncols - 2:
            writeList.append(bpRange)
            index6 += 1
        index2 += 1
    if writeList != []:
        writeList = writeList[::-1]
        writeList.append(sheet.name)
        writeList = writeList[::-1]
        writeListFinal.append(writeList)
for item in writeListFinal:
    write.writerow(item)
...
file.close()
subprocess.call("Data.tsv", shell=True)
